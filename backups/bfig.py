...
for i in range(inter):
	img_lensed = lq_flex_img(img_source,A1,D1)	# 3d gaussian function
	noise_add = np.random.standard_normal\
			((np.shape(img_source)[0],np.shape(img_source)[1]))
	zz = img_lensed + noise_add*0.02*np.sqrt(img_source+200)
...
a = axes([0.1,0.1,0.4,0.8])
a.set_xlim(0.0,3.0)
a.set_ylim(-1.0,1.0)
xlabel(r'${\rm C_W}$')
ylabel(r'$(\bar{F}-F_0)/F_0$, $(\bar{G}-G_0)/G_0$')
